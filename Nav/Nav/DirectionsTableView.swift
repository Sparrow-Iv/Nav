...
extension DirectionsTableView: UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    ...
    label.text = "СЕГМЕНТ #\(section+1)\n\nНачальная точка: \(directionsArray[section].startingAddress)\n"
  ...
  func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
    ...
    label.text = "Конечная точка: \(directionsArray[section].endingAddress)\n\nДистанция: \(miles) миль\n\nОжидаемый путь Время: \(time)"
...
extension DirectionsTableView: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    cell.textLabel?.text = "\(indexPath.row+1). \(instructions) - \(distance) Миль"
